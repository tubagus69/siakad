...
<html lang="en">
  <head>
    ...
    <title>Halaman Transaksi User</title>
  </head>
  <body>
  ...
  <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
    <div class="navbar-nav">
     ...
     <div class="container">
        <div class="row mt-4">
            <div class ="col">
                <h1 style="color: #343a40">Transaksi</h1>
                <img src="<?php echo base_url().'assets/admin/img/uang4.png' ?>" 
	...
	<table class="table table-striped table-bordered" id="table">
		...
		<tbody>
			<?php
				$no=1;
				foreach($transaksi as $jns){ ?>
					<tr>
						<td> <?= $no++; ?> </td>
						<td> <?= $jns->nama_barang; ?> </td>
            <td> <?= $jns->jumlah; ?> </td>
            <td> <?= $jns->harga; ?> </td>
            <td> <?= $jns->harga_total; ?> </td>
            <td> <?= $jns->bayar; ?> </td>
            <td> <?= $jns->kembalian; ?> </td>
            <!-- <td> <?= $jns->tanggal; ?> </td> -->
            <td><?= date('d F Y', strtotime($jns->tanggal)); ?>
					</tr>
				<?php } ?>
		</tbody>
	</table>
  <a class='btn btn-dark'href= "<?= base_url() ?>transaksiUser/post">Tambah</a>
</div>
